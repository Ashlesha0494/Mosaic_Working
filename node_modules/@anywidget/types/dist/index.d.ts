...
export interface AnyModel<T extends ObjectHash = ObjectHash> {
    ...
    off<K extends keyof T>(eventName?: LiteralUnion<`change:${K & string}` | "msg:custom"> | null, callback?: EventHandler | null): void;
    on(eventName: "msg:custom", callback: (msg: any, buffers: DataView[]) => void): void;
    on<K extends `change:${keyof T & string}`>(eventName: K, callback: K extends `change:${infer Key}` ? ChangeEventHandler<T[Key]> : never): void;
