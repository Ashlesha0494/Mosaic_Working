...
function maybeTake(values2, index) {
  return index ? take(values2, index) : values2;
}
function maybeTypedMap(data, f, type) {
  return map(data, (type == null ? void 0 : type.prototype) instanceof TypedArray ? floater(f) : f, type);
}
function maybeTypedArrayify(data, type) {
  return type === void 0 ? arrayify(data) : data instanceof type ? data : type.prototype instanceof TypedArray && !(data instanceof TypedArray) ? type.from(data, coerceNumber) : type.from(data);
}
function floater(f) {
...
function take(values2, index) {
  return map(index, (i) => values2[i], values2.constructor);
}
function taker(f) {
...
function maybeNiceInterval(interval, type) {
  ...
  if (interval && typeof interval.ceil !== "function")
...
function sortValue(value) {
  ...
  if (order === void 0)
    order = negate ? descendingDefined : ascendingDefined;
        ...
        for (const [f, I] of maybeGroup(facet, G)) {
          for (const [y3, gg] of maybeGroup(I, Y)) {
            for (const [x3, g] of maybeGroup(gg, X)) {
              ...
              if (G)
                extent3.z = f;
              if (filter2 && !filter2.reduce(g, extent3))
                continue;
              groupFacet.push(i++);
  ...
  return {
    type: ({ width, height, rotate, precision = 0.15, clip }) => {
      ...
      if (typeof clip === "number")
        (_c = projection2.clipAngle) == null ? void 0 : _c.call(projection2, clip);
...
function applyPosition(channels, scales, { projection: projection2 }) {
  ...
  if (projection2 && (x == null ? void 0 : x.scale) === "x" && (y == null ? void 0 : y.scale) === "y")
    project("x", "y", position3, projection2);
  if (x)
...
function createScaleQ(key, scale2, channels, {
  ...
  if (range3 !== void 0) {
    ...
    const m = (range3 = arrayify(range3)).length;
    if (n !== m) {
...
function coerceType(channels, { domain, ...options }, coerceValues) {
  ...
  for (const c of channels) {
    if (c.value !== void 0) {
      if (domain === void 0)
        domain = (_a = c.value) == null ? void 0 : _a.domain;
...
function applyClip(selection, mark, dimensions, context) {
  ...
  switch (clip) {
    case "frame": {
      ...
      const id = getClipId();
      clipUrl = `url(#${id})`;
      selection = create("svg:g", context).call(
        (g) => g.append("svg:clipPath").attr("id", id).append("rect").attr("x", marginLeft).attr("y", marginTop).attr("width", width - marginRight - marginLeft).attr("height", height - marginTop - marginBottom)
      ).each(function() {
    ...
    case "sphere": {
      ...
      const id = getClipId();
      clipUrl = `url(#${id})`;
      selection.append("clipPath").attr("id", id).append("path").attr("d", path_default(projection2)({ type: "Sphere" }));
      break;
    }
  }
...
function facetIndex(V) {
  ...
  if (!I)
    indexCache.set(V, I = new InternMap(map(V, (v, i) => [v, i])));
  return I;
}
...
function facetAnchorTopEmpty(facets, { y: Y }, { x, y, empty }) {
  ...
  if (i > 0)
    return facetEmpty(facets, x, Y[i - 1]);
}
...
function facetAnchorLeftEmpty(facets, { x: X }, { x, y, empty }) {
  ...
  if (i > 0)
    return facetEmpty(facets, X[i - 1], y);
}
...
var Mark = class {
  ...
  initialize(facets, facetChannels, plotOptions) {
    ...
    if (this.transform != null)
      ({ facets, data } = this.transform(data, facets, plotOptions)), data = arrayify(data);
    if (facets !== void 0)
...
function pointerK(kx, ky, { x, y, px, py, maxRadius = 40, channels, render, ...options } = {}) {
  maxRadius = +maxRadius;
  if (px != null)
    x ?? (x = null), channels = { ...channels, px: { value: px, scale: "x" } };
  if (py != null)
    y ?? (y = null), channels = { ...channels, py: { value: py, scale: "y" } };
  return {
    x,
    y,
    channels,
    ...
    render: composeRender(function(index, scales, values2, dimensions, context, next) {
      ...
      if (x2 == null ? void 0 : x2.bandwidth)
        tx += x2.bandwidth() / 2;
      if (y2 == null ? void 0 : y2.bandwidth)
        ty += y2.bandwidth() / 2;
      ...
      function render2(ii) {
        if (i === ii && s === state.sticky)
          return;
        i = ii;
        s = context.pointerSticky = state.sticky;
        const I = i == null ? [] : [i];
        if (faceted)
          I.fx = index.fx, I.fy = index.fy, I.fi = index.fi;
...
:where(.${className}-ramp text) {
  ...
  if (interpolate) {
    ...
    const thresholdFormat = tickFormat === void 0 ? (d) => d : typeof tickFormat === "string" ? format2(tickFormat) : tickFormat;
...
function axisTextKy(k, anchor, data, {
  ...
  return axisMark(
    ...
    function(scale2, data2, ticks2, tickFormat, channels) {
      ...
      if (text2 === void 0)
        channels.text = inferTextChannel(scale2, data2, ticks2, tickFormat, anchor);
    }
  );
}
...
function axisMark(mark, k, data, properties, options, initialize) {
  ...
  function axisInitializer(data2, facets, _channels, scales, dimensions, context) {
    ...
    if (typeof ticks2 === "string" && hasTemporalDomain(scale2))
      interval = ticks2, ticks2 = void 0;
    if (ticks2 === void 0)
      ticks2 = maybeRangeInterval(interval, scale2.type) ?? inferTickCount(scale2, tickSpacing);
    if (data2 == null) {
      if (isIterable(ticks2)) {
        ...
        if (scale2.ticks) {
          ...
          if (interval2 !== scale2.interval)
            data2 = inclusiveRange(interval2, ...extent(data2));
        }
        if (interval2 === scale2.interval) {
      ...
      if (!scale2.ticks && data2.length && data2 !== domain) {
        ...
        if (!data2.length)
          warn(`Warning: the ${k}-axis ticks appear to not align with the scale domain, resulting in no ticks. Try different ticks?`);
      }
      if (k === "y" || k === "x") {
        facets = [range2(data2)];
      } else {
...
function inferTickFormat(scale2, data, ticks2, tickFormat, anchor) {
  return typeof tickFormat === "function" ? tickFormat : tickFormat === void 0 && data && isTemporal(data) ? inferTimeFormat(scale2.type, data, anchor) ?? formatDefault : scale2.tickFormat ? scale2.tickFormat(typeof ticks2 === "number" ? ticks2 : null, tickFormat) : tickFormat === void 0 ? formatDefault : typeof tickFormat === "string" ? (isTemporal(scale2.domain()) ? utcFormat : format2)(tickFormat) : constant(tickFormat);
}
...
function formatAxisLabel(k, scale2, { anchor, label = scale2.label, labelAnchor, labelArrow } = {}) {
  if (label == null || label.inferred && hasTemporalDomain(scale2) && /^(date|time|year)$/i.test(label))
    return;
  label = String(label);
  if (labelArrow === "auto")
...
var Frame = class extends Mark {
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    return create(anchor ? "svg:line" : "svg:rect", context).datum(0).call(applyIndirectStyles, this, dimensions, context).call(applyDirectStyles, this).call(applyChannelStyles, this, channels).call(applyTransform, this, {}).call(
      anchor === "left" ? (line2) => line2.attr("x1", x1).attr("x2", x1).attr("y1", y1).attr("y2", y2) : anchor === "right" ? (line2) => line2.attr("x1", x2).attr("x2", x2).attr("y1", y1).attr("y2", y2) : anchor === "top" ? (line2) => line2.attr("x1", x1).attr("x2", x2).attr("y1", y1).attr("y2", y1) : anchor === "bottom" ? (line2) => line2.attr("x1", x1).attr("x2", x2).attr("y1", y2).attr("y2", y2) : (rect2) => rect2.attr("x", x1).attr("y", y1).attr("width", x2 - x1).attr("height", y2 - y1).attr("rx", rx).attr("ry", ry)
    ).node();
  }
};
...
var Tip = class extends Mark {
  constructor(data, options = {}) {
    ...
    if (options.title === void 0 && isIterable(data) && isTextual(data))
      options = { ...options, title: identity2 };
    const {
      x,
      y,
      x1,
  ...
  render(index, scales, values2, dimensions, context) {
    ...
    function renderLine(selection, { label, value, color: color2, opacity: opacity2 }) {
      ...
      if (value)
        line2.append(() => document.createTextNode(value));
      if (swatch)
        line2.append("tspan").text(" ■").attr("fill", color2).attr("fill-opacity", opacity2).style("user-select", "none");
      if (title)
...
function getSourceChannels({ channels }, scales) {
  ...
  for (const key in sources) {
    ...
    if (typeof format4 === "string") {
      ...
      this.format[key] = (isTemporal(value) ? utcFormat : format2)(format4);
...
:where(.${className} tspan) {
  ...
  if (figured) {
    ...
    if (title != null)
      figure.append(createTitleElement(document, title, "h2"));
    if (subtitle != null)
      figure.append(createTitleElement(document, subtitle, "h3"));
    figure.append(...legends, svg);
    if (caption != null)
      figure.append(createFigcaption(document, caption));
  }
...
function maybeTopFacet(facet, options) {
  ...
  const channels = {};
  if (x != null)
    channels.fx = createChannel(data, { value: x, scale: "fx" });
  if (y != null)
    channels.fy = createChannel(data, { value: y, scale: "fy" });
  applyScaleTransforms(channels, options);
...
function maybeMarkFacet(mark, topFacetState, options) {
  ...
  if (data.length > 0 && (groups.size > 1 || groups.size === 1 && channels.fx && channels.fy && [...groups][0][1].size > 1) && ((_a = arrayify(mark.data)) == null ? void 0 : _a.length) === data.length) {
    warn(
...
function inferAxes(marks2, channelsByScale, options) {
  ...
  if (projection2 || !isScaleOptions(x) && !hasPositionChannel("x", marks2))
    xAxis = xGrid = null;
  if (projection2 || !isScaleOptions(y) && !hasPositionChannel("y", marks2))
    yAxis = yGrid = null;
...
function maybeAxis(axes, axis, axisType, primary, secondary, defaults23, options) {
  if (!axis)
    return;
  const both = isBoth(axis);
  options = axisOptions(both ? primary : axis, defaults23, options);
...
function maybeIdentityX(options = {}) {
  return hasX(options) ? options : { ...options, x: identity2 };
}
function maybeIdentityY(options = {}) {
  return hasY(options) ? options : { ...options, y: identity2 };
}
...
function exclusiveFacets(data, facets) {
  ...
  for (const facet of facets) {
    for (let k = 0, m = facet.length; k < m; ++k) {
      const i = facet[k];
      if (O[i])
        facet[k] = j, data[j] = data[i], R[j] = i, ++j;
      else
        R[i] = i;
      O[i] = 1;
    }
...
function stack(x, y = one, kx, ky, { offset: offset2, order, reverse: reverse3 }, options) {
  if (y === null)
    throw new Error(`stack requires ${ky}`);
...
function maybeOrder2(order, offset2, ky) {
  if (order === void 0 && offset2 === offsetWiggle)
    return orderInsideOut(ascendingDefined);
  if (order == null)
    return;
...
var Link = class extends Mark {
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    return create("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, scales).call(
      (g) => g.selectAll().data(index).enter().append("path").call(applyDirectStyles, this).attr(
        "d",
        curve === curveAuto && context.projection ? sphereLink(context.projection, X1, Y1, X2, Y2) : (i) => {
          const p = pathRound();
          const c = curve(p);
          c.lineStart();
          c.point(X1[i], Y1[i]);
...
function sphereLink(projection2, X1, Y1, X2, Y2) {
...
function barX(data, options = {}) {
  if (!hasXY(options))
    options = { ...options, y: indexOf, x2: identity2 };
  return new BarX(data, maybeStackX(maybeIntervalX(maybeIdentityX(options))));
}
function barY(data, options = {}) {
  if (!hasXY(options))
    options = { ...options, x: indexOf, y2: identity2 };
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    return create("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, scales).call(
      (g) => g.selectAll().data(groupIndex(index, [X, Y], this, channels)).enter().append("path").call(applyDirectStyles, this).call(applyGroupedChannelStyles, this, channels).call(applyGroupedMarkers, this, channels, context).attr(
        "d",
        curve === curveAuto && context.projection ? sphereLine(context.projection, X, Y) : line_default().curve(curve).defined((i) => i >= 0).x((i) => X[i]).y((i) => Y[i])
      )
    ).node();
  }
};
function sphereLine(projection2, X, Y) {
  const path = path_default(projection2);
  X = coerceNumbers(X);
  Y = coerceNumbers(Y);
  return (I) => {
...
var Rect = class extends Mark {
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    const bx = ((x == null ? void 0 : x.bandwidth) ? x.bandwidth() : 0) - insetLeft - insetRight;
    const by = ((y == null ? void 0 : y.bandwidth) ? y.bandwidth() : 0) - insetTop - insetBottom;
    return create("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, {}, 0, 0).call(
      (g) => g.selectAll().data(index).enter().append("rect").call(applyDirectStyles, this).attr(
        "x",
        X1 && (projection2 || !isCollapsed(x)) ? X2 ? (i) => Math.min(X1[i], X2[i]) + insetLeft : (i) => X1[i] + insetLeft : marginLeft + insetLeft
      ).attr(
        "y",
        Y1 && (projection2 || !isCollapsed(y)) ? Y2 ? (i) => Math.min(Y1[i], Y2[i]) + insetTop : (i) => Y1[i] + insetTop : marginTop + insetTop
      ).attr(
        "width",
        X1 && (projection2 || !isCollapsed(x)) ? X2 ? (i) => Math.max(0, Math.abs(X2[i] - X1[i]) + bx) : bx : width - marginRight - marginLeft - insetRight - insetLeft
      ).attr(
        "height",
        Y1 && (projection2 || !isCollapsed(y)) ? Y2 ? (i) => Math.max(0, Math.abs(Y1[i] - Y2[i]) + by) : by : height - marginTop - marginBottom - insetTop - insetBottom
...
function rectX(data, options = {}) {
  if (!hasXY(options))
    options = { ...options, y: indexOf, x2: identity2, interval: 1 };
  return new Rect(data, maybeStackX(maybeTrivialIntervalY(maybeIdentityX(options))));
}
function rectY(data, options = {}) {
  if (!hasXY(options))
    options = { ...options, x: indexOf, y2: identity2, interval: 1 };
...
function autoSpec(data, options) {
  ...
  if (transformImpl === bin || transformImpl === binX)
    markOptions2.x = { value: X, ...xOptions };
  if (transformImpl === bin || transformImpl === binY)
    markOptions2.y = { value: Y, ...yOptions };
  if (xZero === void 0)
...
function normalizeOptions({ x, y, color: color2, size, fx, fy, mark } = {}) {
  ...
  if (!isOptions(y))
    y = makeOptions(y);
  if (!isOptions(color2))
    color2 = isColor(color2) ? { color: color2 } : makeOptions(color2);
  if (!isOptions(size))
...
function mapX(mapper, options = {}) {
  let { x, x1, x2 } = options;
  if (x === void 0 && x1 === void 0 && x2 === void 0)
    options = { ...options, x: x = identity2 };
  const outputs = {};
  if (x != null)
    outputs.x = mapper;
  if (x1 != null)
...
function mapY(mapper, options = {}) {
  let { y, y1, y2 } = options;
  if (y === void 0 && y1 === void 0 && y2 === void 0)
    options = { ...options, y: y = identity2 };
  const outputs = {};
  if (y != null)
    outputs.y = mapper;
  if (y1 != null)
...
function maybeMap(map3) {
  ...
  if (typeof map3.mapIndex === "function")
    return map3;
  if (typeof map3.map === "function" && isObject(map3))
    return mapMap(map3);
  if (typeof map3 === "function")
    return mapFunction(taker(map3));
...
function bollinger({ n = defaults14.n, k = 0, strict = defaults14.strict, anchor = defaults14.anchor } = {}) {
...
function boxX(data, {
  ...
  return marks(
    ...
    dot(data, map2({ x: oqr }, { x, y, z: y, stroke, strokeOpacity, ...options }))
  );
}
function boxY(data, {
  y = identity2,
  ...
  return marks(
    ...
    dot(data, map2({ y: oqr }, { x, y, z: x, stroke, strokeOpacity, ...options }))
  );
}
      ...
      for (let x = Math.floor(x1); x < x2; ++x) {
        for (let y = Math.floor(y1); y < y2; ++y) {
          ...
          if (gc < 0)
            continue;
          const i2 = x + width * y;
          W[i2] = mix(va, ga, vb, gb, vc, gc, x, y);
          S[i2] = 1;
        }
      }
    }
...
function crosshairK(pointer2, data, options = {}) {
  ...
  const M = [];
  if (x != null)
    M.push(ruleX(data, ruleOptions("x", { ...p, inset: -6 }, options)));
  if (y != null)
    M.push(ruleY(data, ruleOptions("y", { ...p, inset: -6 }, options)));
  if (x != null)
    M.push(text(data, textOptions("x", { ...p, dy: 9, frameAnchor: "bottom", lineAnchor: "top" }, options)));
  if (y != null)
    M.push(text(data, textOptions("y", { ...p, dx: -9, frameAnchor: "left", textAnchor: "end" }, options)));
  for (const m of M)
...
var Voronoi = class extends Mark {
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    function cells(index2) {
      ...
      select_default(this).selectAll().data(index2).enter().append("path").call(applyDirectStyles, mark).attr("d", (_, i) => voronoi2.renderCell(i)).call(applyChannelStyles, mark, channels);
    }
    return create("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, { x: X && x, y: Y && y }).call(
      Z ? (g) => g.selectAll().data(group(index, (i) => Z[i]).values()).enter().append("g").each(cells) : (g) => g.datum(index).each(cells)
    ).node();
  }
};
...
function differenceY(data, {
  ...
  y1,
  y2,
  x = x1 === void 0 && x2 === void 0 ? indexOf : void 0,
  y = y1 === void 0 && y2 === void 0 ? identity2 : void 0,
  fill,
  // ignored
  positiveFill = "#3ca951",
  negativeFill = "#4269d0",
...
function clipDifferenceY(positive2) {
  return (index, scales, channels, dimensions, context, next) => {
    ...
    (positive2 === inferScaleOrder(scales.y) < 0 ? y1 : y2).fill(height);
...
function dodge(y, x, anchor, padding, r, options) {
  ...
  return initializer(options, function(data, facets, channels, scales, dimensions, context) {
    ...
    if (R)
      R = valueof(R.value, scales[R.scale] || identity2, Float64Array);
...
function normalize(basis) {
  ...
  if (typeof basis === "function")
    return normalizeBasis(taker(basis));
