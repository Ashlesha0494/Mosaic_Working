...
function link_default(links) {
  var id = index2, strength = defaultStrength, strengths, distance = constant_default2(30), distances, nodes, count3, bias, random, iterations2 = 1;
  if (links == null)
    links = [];
...
function x_default2(x4) {
  ...
  if (typeof x4 !== "function")
    x4 = constant_default2(x4 == null ? 0 : +x4);
  function force(alpha) {
    for (var i = 0, n = nodes.length, node; i < n; ++i) {
  ...
  force.x = function(_) {
    return arguments.length ? (x4 = typeof _ === "function" ? _ : constant_default2(+_), initialize(), force) : x4;
  };
  return force;
}
...
function y_default2(y4) {
  ...
  if (typeof y4 !== "function")
    y4 = constant_default2(y4 == null ? 0 : +y4);
  function force(alpha) {
    for (var i = 0, n = nodes.length, node; i < n; ++i) {
  ...
  force.y = function(_) {
    return arguments.length ? (y4 = typeof _ === "function" ? _ : constant_default2(+_), initialize(), force) : y4;
  };
  return force;
}
...
function areaPoint(lambda, phi2) {
  ...
  var dLambda = lambda - lambda0, sdLambda = dLambda >= 0 ? 1 : -1, adLambda = sdLambda * dLambda, cosPhi = cos(phi2), sinPhi = sin(phi2), k2 = sinPhi0 * sinPhi, u4 = cosPhi0 * cosPhi + k2 * cos(adLambda), v2 = k2 * sdLambda * sin(adLambda);
...
function linePoint(lambda, phi2) {
  ...
  if (p0) {
    ...
    var delta = lambda - lambda2, sign3 = delta > 0 ? 1 : -1, lambdai = inflection[0] * degrees3 * sign3, phii, antimeridian = abs(delta) > 180;
...
function centroidLinePoint(lambda, phi2) {
  lambda *= radians2, phi2 *= radians2;
  var cosPhi = cos(phi2), x4 = cosPhi * cos(lambda), y4 = cosPhi * sin(lambda), z = sin(phi2), w = atan2(sqrt((w = y0 * z - z0 * y4) * w + (w = z0 * x4 - x0 * z) * w + (w = x0 * y4 - y0 * x4) * w), x0 * x4 + y0 * y4 + z0 * z);
  W1 += w;
  X1 += w * (x0 + (x0 = x4));
  Y1 += w * (y0 + (y0 = y4));
  Z1 += w * (z0 + (z0 = z));
...
function centroidRingPoint(lambda, phi2) {
  lambda *= radians2, phi2 *= radians2;
  var cosPhi = cos(phi2), x4 = cosPhi * cos(lambda), y4 = cosPhi * sin(lambda), z = sin(phi2), cx = y0 * z - z0 * y4, cy = z0 * x4 - x0 * z, cz = x0 * y4 - y0 * x4, m3 = hypot(cx, cy, cz), w = asin(m3), v2 = m3 && -w / m3;
  X2.add(v2 * cx);
  Y2.add(v2 * cy);
  Z2.add(v2 * cz);
  W1 += w;
...
function rotationPhiGamma(deltaPhi, deltaGamma) {
  ...
  function rotation(lambda, phi2) {
    var cosPhi = cos(phi2), x4 = cos(lambda) * cosPhi, y4 = sin(lambda) * cosPhi, z = sin(phi2), k2 = z * cosDeltaPhi + x4 * sinDeltaPhi;
    return [
  ...
  rotation.invert = function(lambda, phi2) {
    var cosPhi = cos(phi2), x4 = cos(lambda) * cosPhi, y4 = sin(lambda) * cosPhi, z = sin(phi2), k2 = z * cosDeltaGamma - y4 * sinDeltaGamma;
    return [
...
function circleRadius(cosRadius, point6) {
  ...
  return ((-point6[2] < 0 ? -radius : radius) + tau3 - epsilon5) % tau3;
}
function circle_default() {
  var center = constant_default3([0, 0]), radius = constant_default3(90), precision = constant_default3(6), ring, rotate, stream = { point: point6 };
...
function pointEqual_default(a4, b) {
  return abs(a4[0] - b[0]) < epsilon5 && abs(a4[1] - b[1]) < epsilon5;
}
...
function longitude(point6) {
  return abs(point6[0]) <= pi2 ? point6[0] : sign(point6[0]) * ((abs(point6[0]) + pi2) % tau3 - pi2);
}
function polygonContains_default(polygon, point6) {
  var lambda = longitude(point6), phi2 = point6[1], sinPhi = sin(phi2), normal = [sin(lambda), -cos(lambda), 0], angle2 = 0, winding = 0;
  var sum4 = new Adder();
  if (sinPhi === 1)
    phi2 = halfPi + epsilon5;
  else if (sinPhi === -1)
    phi2 = -halfPi - epsilon5;
  for (var i = 0, n = polygon.length; i < n; ++i) {
    ...
    for (var j = 0; j < m3; ++j, lambda04 = lambda12, sinPhi03 = sinPhi1, cosPhi03 = cosPhi1, point0 = point1) {
      var point1 = ring[j], lambda12 = longitude(point1), phi12 = point1[1] / 2 + quarterPi, sinPhi1 = sin(phi12), cosPhi1 = cos(phi12), delta = lambda12 - lambda04, sign3 = delta >= 0 ? 1 : -1, absDelta = sign3 * delta, antimeridian = absDelta > pi2, k2 = sinPhi03 * sinPhi1;
      sum4.add(atan2(k2 * sign3 * sin(absDelta), cosPhi03 * cosPhi1 + k2 * cos(absDelta)));
...
function compareIntersection(a4, b) {
  return ((a4 = a4.x)[0] < 0 ? a4[1] - halfPi - epsilon5 : halfPi - a4[1]) - ((b = b.x)[0] < 0 ? b[1] - halfPi - epsilon5 : halfPi - b[1]);
}
  ...
  return {
    ...
    point: function(lambda12, phi12) {
      ...
      if (abs(delta - pi2) < epsilon5) {
        ...
        if (abs(lambda04 - sign0) < epsilon5)
          lambda04 -= sign0 * epsilon5;
        if (abs(lambda12 - sign1) < epsilon5)
...
function clipAntimeridianIntersect(lambda04, phi02, lambda12, phi12) {
  ...
  return abs(sinLambda0Lambda1) > epsilon5 ? atan((sin(phi02) * (cosPhi1 = cos(phi12)) * sin(lambda12) - sin(phi12) * (cosPhi03 = cos(phi02)) * sin(lambda04)) / (cosPhi03 * cosPhi1 * sinLambda0Lambda1)) : (phi02 + phi12) / 2;
}
...
function circle_default2(radius) {
  var cr = cos(radius), delta = 6 * radians2, smallRadius = cr > 0, notHemisphere = abs(cr) > epsilon5;
  ...
  function intersect2(a4, b, two) {
    ...
    var delta2 = lambda12 - lambda04, polar = abs(delta2 - pi2) < epsilon5, meridian = polar || delta2 < epsilon5;
    if (!polar && phi12 < phi02)
      z = phi02, phi02 = phi12, phi12 = z;
    if (meridian ? polar ? phi02 + phi12 > 0 ^ q[1] < (abs(q[0] - lambda04) < epsilon5 ? phi02 : phi12) : phi02 <= q[1] && q[1] <= phi12 : delta2 > pi2 ^ (lambda04 <= q[0] && q[0] <= lambda12)) {
...
function clipRectangle(x06, y06, x12, y12) {
  ...
  function corner(p, direction) {
    return abs(p[0] - x06) < epsilon5 ? direction > 0 ? 0 : 3 : abs(p[0] - x12) < epsilon5 ? direction > 0 ? 2 : 1 : abs(p[1] - y06) < epsilon5 ? direction > 0 ? 1 : 0 : direction > 0 ? 3 : 2;
  }
...
function lengthPoint(lambda, phi2) {
  lambda *= radians2, phi2 *= radians2;
  var sinPhi = sin(phi2), cosPhi = cos(phi2), delta = abs(lambda - lambda03), cosDelta = cos(delta), sinDelta = sin(delta), x4 = cosPhi * sinDelta, y4 = cosPhi02 * sinPhi - sinPhi02 * cosPhi * cosDelta, z = sinPhi02 * sinPhi + cosPhi02 * cosPhi * cosDelta;
...
function interpolate_default(a4, b) {
  var x06 = a4[0] * radians2, y06 = a4[1] * radians2, x12 = b[0] * radians2, y12 = b[1] * radians2, cy0 = cos(y06), sy0 = sin(y06), cy1 = cos(y12), sy1 = sin(y12), kx0 = cy0 * cos(x06), ky0 = cy0 * sin(x06), kx1 = cy1 * cos(x12), ky1 = cy1 * sin(x12), d = 2 * asin(sqrt(haversin(y12 - y06) + cy0 * cy1 * haversin(x12 - x06))), k2 = sin(d);
  var interpolate = d ? function(t) {
    var B3 = sin(t *= d) / k2, A5 = sin(d - t) / k2, x4 = A5 * kx0 + B3 * kx1, y4 = A5 * ky0 + B3 * ky1, z = A5 * sy0 + B3 * sy1;
    return [
...
function resample(project, delta2) {
  function resampleLineTo(x06, y06, lambda04, a0, b0, c0, x12, y12, lambda12, a1, b1, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a4 = a0 + a1, b = b0 + b1, c4 = c0 + c1, m3 = sqrt(a4 * a4 + b * b + c4 * c4), phi2 = asin(c4 /= m3), lambda22 = abs(abs(c4) - 1) < epsilon5 || abs(lambda04 - lambda12) < epsilon5 ? (lambda04 + lambda12) / 2 : atan2(b, a4), p = project(lambda22, phi2), x22 = p[0], y22 = p[1], dx2 = x22 - x06, dy2 = y22 - y06, dz = dy * dx2 - dx * dy2;
...
function scaleTranslateRotate(k2, dx, dy, sx, sy, alpha) {
  ...
  var cosAlpha = cos(alpha), sinAlpha = sin(alpha), a4 = cosAlpha * k2, b = sinAlpha * k2, ai = cosAlpha / k2, bi = sinAlpha / k2, ci = (sinAlpha * dy - cosAlpha * dx) / k2, fi = (sinAlpha * dx + cosAlpha * dy) / k2;
  function transform(x4, y4) {
    x4 *= sx;
    y4 *= sy;
...
function conicEqualAreaRaw(y06, y12) {
  ...
  project.invert = function(x4, y4) {
    var r0y = r0 - y4, l = atan2(x4, abs(r0y)) * sign(r0y);
    if (r0y * n < 0)
...
function azimuthalRaw(scale2) {
  return function(x4, y4) {
    var cx = cos(x4), cy = cos(y4), k2 = scale2(cx * cy);
    if (k2 === Infinity)
      return [2, 0];
    return [
      k2 * cy * sin(x4),
...
function azimuthalInvert(angle2) {
  return function(x4, y4) {
    var z = sqrt(x4 * x4 + y4 * y4), c4 = angle2(z), sc = sin(c4), cc2 = cos(c4);
    return [
      atan2(x4 * sc, z * cc2),
      asin(z && y4 * sc / z)
    ];
...
function mercatorProjection(project) {
  ...
  function reclip() {
    var k2 = pi2 * scale2(), t = m3(rotation_default(m3.rotate()).invert([0, 0]));
...
function conicConformalRaw(y06, y12) {
  ...
  function project(x4, y4) {
    ...
    return [r * sin(n * x4), f - r * cos(n * x4)];
  }
  project.invert = function(x4, y4) {
    var fy = f - y4, r = sign(n) * sqrt(x4 * x4 + fy * fy), l = atan2(x4, abs(fy)) * sign(fy);
    if (fy * n < 0)
...
function conicEquidistantRaw(y06, y12) {
  ...
  project.invert = function(x4, y4) {
    var gy = g - y4, l = atan2(x4, abs(gy)) * sign(gy);
    if (gy * n < 0)
...
equalEarthRaw.invert = function(x4, y4) {
  ...
  return [
    M * x4 * (A1 + 3 * A2 * l2 + l6 * (7 * A3 + 9 * A4 * l2)) / cos(l),
    asin(sin(l) / M)
  ];
};
...
function gnomonicRaw(x4, y4) {
  var cy = cos(y4), k2 = cos(x4) * cy;
  return [cy * sin(x4) / k2, sin(y4) / k2];
}
...
function cornerTangents(x06, y06, x12, y12, r1, rc, cw) {
  var x01 = x06 - x12, y01 = y06 - y12, lo = (cw ? rc : -rc) / sqrt3(x01 * x01 + y01 * y01), ox = lo * y01, oy = -lo * x01, x11 = x06 + ox, y11 = y06 + oy, x10 = x12 + ox, y10 = y12 + oy, x004 = (x11 + x10) / 2, y004 = (y11 + y10) / 2, dx = x10 - x11, dy = y10 - y11, d2 = dx * dx + dy * dy, r = r1 - rc, D3 = x11 * y10 - x10 * y11, d = (dy < 0 ? -1 : 1) * sqrt3(max2(0, r * r * d2 - D3 * D3)), cx0 = (D3 * dy - dx * d) / d2, cy0 = (-D3 * dx - dy * d) / d2, cx1 = (D3 * dy + dx * d) / d2, cy1 = (-D3 * dx + dy * d) / d2, dx0 = cx0 - x004, dy0 = cy0 - y004, dx1 = cx1 - x004, dy1 = cy1 - y004;
...
function arc_default() {
  var innerRadius = arcInnerRadius, outerRadius = arcOuterRadius, cornerRadius = constant_default5(0), padRadius = null, startAngle = arcStartAngle, endAngle = arcEndAngle, padAngle = arcPadAngle, context = null, path2 = withPath(arc);
  function arc() {
    var buffer, r, r0 = +innerRadius.apply(this, arguments), r1 = +outerRadius.apply(this, arguments), a0 = startAngle.apply(this, arguments) - halfPi2, a1 = endAngle.apply(this, arguments) - halfPi2, da2 = abs2(a1 - a0), cw = a1 > a0;
    if (!context)
    ...
    if (!(r1 > epsilon6))
      ...
      var a01 = a0, a11 = a1, a00 = a0, a10 = a1, da0 = da2, da1 = da2, ap = padAngle.apply(this, arguments) / 2, rp = ap > epsilon6 && (padRadius ? +padRadius.apply(this, arguments) : sqrt3(r0 * r0 + r1 * r1)), rc = min2(abs2(r1 - r0) / 2, +cornerRadius.apply(this, arguments)), rc0 = rc, rc1 = rc, t03, t13;
      if (rp > epsilon6) {
        var p02 = asin2(rp / r0 * sin2(ap)), p1 = asin2(rp / r1 * sin2(ap));
        if ((da0 -= p02 * 2) > epsilon6)
      ...
      if (rc > epsilon6) {
        var x11 = r1 * cos2(a11), y11 = r1 * sin2(a11), x004 = r0 * cos2(a00), y004 = r0 * sin2(a00), oc;
        if (da2 < pi3) {
          if (oc = intersect(x01, y01, x004, y004, x11, y11, x10, y10)) {
            var ax = x01 - oc[0], ay = y01 - oc[1], bx = x11 - oc[0], by = y11 - oc[1], kc = 1 / sin2(acos2((ax * bx + ay * by) / (sqrt3(ax * ax + ay * ay) * sqrt3(bx * bx + by * by))) / 2), lc = sqrt3(oc[0] * oc[0] + oc[1] * oc[1]);
  ...
  arc.centroid = function() {
    var r = (+innerRadius.apply(this, arguments) + +outerRadius.apply(this, arguments)) / 2, a4 = (+startAngle.apply(this, arguments) + +endAngle.apply(this, arguments)) / 2 - pi3 / 2;
...
function line_default2(x4, y4) {
  ...
  line.x = function(_) {
    return arguments.length ? (x4 = typeof _ === "function" ? _ : constant_default5(+_), line) : x4;
  };
  line.y = function(_) {
    return arguments.length ? (y4 = typeof _ === "function" ? _ : constant_default5(+_), line) : y4;
  };
...
function area_default3(x06, y06, y12) {
  var x12 = null, defined = constant_default5(true), context = null, curve = linear_default, output = null, path2 = withPath(area);
  ...
  area.x = function(_) {
    return arguments.length ? (x06 = typeof _ === "function" ? _ : constant_default5(+_), x12 = null, area) : x06;
  };
  area.x0 = function(_) {
    return arguments.length ? (x06 = typeof _ === "function" ? _ : constant_default5(+_), area) : x06;
  };
  area.x1 = function(_) {
    return arguments.length ? (x12 = _ == null ? null : typeof _ === "function" ? _ : constant_default5(+_), area) : x12;
  };
  area.y = function(_) {
    return arguments.length ? (y06 = typeof _ === "function" ? _ : constant_default5(+_), y12 = null, area) : y06;
  };
  area.y0 = function(_) {
    return arguments.length ? (y06 = typeof _ === "function" ? _ : constant_default5(+_), area) : y06;
  };
  area.y1 = function(_) {
...
function pie_default() {
  var value = identity_default4, sortValues = descending_default, sort2 = null, startAngle = constant_default5(0), endAngle = constant_default5(tau4), padAngle = constant_default5(0);
  function pie(data) {
    var i, n = (data = array_default3(data)).length, j, k2, sum4 = 0, index3 = new Array(n), arcs = new Array(n), a0 = +startAngle.apply(this, arguments), da2 = Math.min(tau4, Math.max(-tau4, endAngle.apply(this, arguments) - a0)), a1, p = Math.min(Math.abs(da2) / n, padAngle.apply(this, arguments)), pa = p * (da2 < 0 ? -1 : 1), v2;
    for (i = 0; i < n; ++i) {
...
function areaRadial_default() {
  var a4 = area_default3().curve(curveRadialLinear), c4 = a4.curve, x06 = a4.lineX0, x12 = a4.lineX1, y06 = a4.lineY0, y12 = a4.lineY1;
...
function link2(curve) {
  ...
  link3.x = function(_) {
    return arguments.length ? (x4 = typeof _ === "function" ? _ : constant_default5(+_), link3) : x4;
  };
  link3.y = function(_) {
    return arguments.length ? (y4 = typeof _ === "function" ? _ : constant_default5(+_), link3) : y4;
  };
...
function Symbol2(type, size) {
  ...
  type = typeof type === "function" ? type : constant_default5(type || circle_default3);
...
export {
  ...
  area_default3 as area_default2,
  pie_default,
  lineRadial_default,
  areaRadial_default,
  pointRadial_default,
...
//# sourceMappingURL=chunk-DOCCAXVD.js.map
