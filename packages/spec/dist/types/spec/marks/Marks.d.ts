 ...
 */
export type ChannelName = 'ariaLabel' | 'fill' | 'fillOpacity' | 'fontSize' | 'fx' | 'fy' | 'geometry' | 'height' | 'href' | 'length' | 'opacity' | 'path' | 'r' | 'rotate' | 'src' | 'stroke' | 'strokeOpacity' | 'strokeWidth' | 'symbol' | 'text' | 'title' | 'weight' | 'width' | 'x' | 'x1' | 'x2' | 'y' | 'y1' | 'y2' | 'z' | (string & Record<never, never>);
 ...
 */
export type ChannelValue = any[] | (string & Record<never, never>) | Date | number | boolean | null | Transform | SQLExpression | AggregateExpression;
/**
 ...
 */
export type StackOffsetName = 'center' | 'normalize' | 'wiggle' | ('expand' & Record<never, never>) | ('silhouette' & Record<never, never>);
/**
